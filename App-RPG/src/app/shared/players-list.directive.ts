...
import { MulticastOperator } from 'rxjs/operators/multicast';
...
export class PlayersListDirective implements OnInit {
  ...
  generateRows() {
    ...
    if (this.acceptedPlayers.length < 5)
      noRows = 1;
    else { 
    ...
    for (let i = 0; i < noRows; i++) {
      ...
      if (i == noRows - 1)
        playersInRow = reminder;

      for (let j = 0; j < playersInRow; j++, index++) {

        const cardContainer = this.renderer.createElement("div");
        let x = 12 / playersInRow;
        this.renderer.setAttribute(cardContainer, "class", "card-container col-xs-" + x.toString());
        this.renderer.setAttribute(cardContainer, "id", "container" + index.toString());
        ...
        if (this.acceptedPlayers[index].photo)
          this.renderer.setAttribute(photo, "src", this.acceptedPlayers[index].photo);
        else 
          this.renderer.setAttribute(photo, "src", "assets/unknown.png");
  ...
  onMouseEnter(target: HTMLElement, event: MouseEvent) {
    if (target.id && !isNaN(Number(target.id))) {
      ...
      this.mouse.setOrigin(c.offsetLeft, c.offsetTop, this.offsetWidth, this.offsetHeight);
      this.update(event.clientX, event.clientY, this.offsetWidth, this.offsetHeight);
    }
  }
